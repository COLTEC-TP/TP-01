...
public class EditMovie extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final int idChange = getIntent().getIntExtra("id", 0);

        DataBaseController crud = new DataBaseController(getBaseContext());
        final Movie movie = crud.loadData(idChange);

        name.setText(movie.getName());
        autor.setText(movie.getDirector());
        ...
        ArrayAdapter<String> adapterGenre = new ArrayAdapter<>(EditMovie.this,
                android.R.layout.simple_spinner_item, genres);
        adapterGenre.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        genre.setAdapter(adapterGenre);

        for(int i = 0; i < genres.length; i++) {
            if(genres[i].equals(movie.getGenre())) {
        ...
        saveChanges.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                DataBaseController crud = new DataBaseController(getBaseContext());
                boolean result = crud.update(
                        new Movie(name.getText().toString(),
                                autor.getText().toString(),
                                genre.getSelectedItem().toString(),
                                age.getSelectedItemPosition(),
                                Integer.parseInt(date.getText().toString())), idChange);
